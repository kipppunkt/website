...
kipppunkt provides two CLI commands: `start` to launch the orchestrator, and `ingress` to load tasks into a running orchestrator.
...
```bash
IS_SANDBOX=1 kipppunkt-build start \
  --command "claude -p {prompt} --dangerously-skip-permissions"
```
:::

:::caution
The commands in this section run the agent **unsandboxed**. For a more permanent and secure setup, see [Sandboxing by harness](/reference/sandboxing-by-harness/) and [Sandbox with Docker](/reference/sandbox-with-docker/).
:::
...
```bash
kipppunkt-build ingress --requirements <requirements.json> [options]
```
